...
| FR-027 | An authorized Conductor may ask a Performer to run a script the Performer already declared, via `omakure node cue`; the Cue names a script and never carries one, and the outcome returns on the existing `run-completed` Signal correlated by a run id derived from the Cue id. | `src/remote_cue.rs`, `src/direct_service.rs`, `src/cli/node.rs`, `docs/internal/remote-cue-contract.md` |
| FR-028 | A Cue is accepted only if all five gates pass, each read from the receiver's own registry and configuration and never from the message: remote Cues enabled, sender is an active conductor, sender holds `remote-run` and `notifications`, and the script is declared in `trust.remote_cue_scripts` or installed by a battery in `trust.remote_cue_batteries`. | `src/remote_cue.rs`, `src/domain/node_config.rs` |
| FR-029 | A publisher signs a versioned set of scripts as one baseline; a Conductor delivers it over the session it already holds; a Performer installs the whole set or none of it. Delivery carries its own size bound so one push fits one frame. | `src/baseline.rs`, `src/baseline_publisher.rs`, `src/baseline_push.rs`, `src/operations/baseline.rs`, `docs/internal/baseline-delivery.md` |
| FR-030 | A baseline installs only if two independent authorities hold: the sender is an active conductor holding `baseline-push`, and the manifest verifies under a publisher named in the receiver's own `trust.baseline_publishers`. The sender's standing is re-read immediately before the write. | `src/baseline_push.rs`, `src/domain/node_config.rs` |
| FR-031 | A Performer reports the identity of the baseline it recorded installing beside the identity recomputed from that set on its disk; the Conductor's fleet projection compares them and reports `unknown`, `none`, `in_sync`, or `drifted` through `omakure node health` and `GET /v1/node/health`. | `src/health_plane/report.rs`, `src/health_plane.rs`, `src/operations/health.rs`, `src/operations/baseline.rs` |
| FR-032 | Each node retains exactly one previous baseline and can be put back on it through `omakure node baseline rollback --confirmed` or `POST /v1/node/baseline/rollback`; the retained set is re-verified against the publishers that node names today, and a node with nothing retained refuses. | `src/operations/baseline.rs`, `src/cli/node.rs`, `src/cli/api.rs` |
...
| NFR-006 | Release CI tests the eight x86_64/aarch64 target assets, denies clippy warnings, checks formatting, verifies binary-only archives, and requires matching release notes. | `.github/workflows/ci.yml`, `.github/workflows/release.yml`, `tests/packaging_smoke.rs` |
...
| NFR-010 | Every certification wait, retry, Docker command, curl, and sqlite query is explicitly bounded, and cleanup is verified after success, failure, interrupt, and partial startup; inspection errors fail closed. | `.scripts/health-plane-certification.sh`, `.scripts/health-plane-certification-cleanup-test.sh` |
...
| BR-013 | A baseline's identity is derived from its entry list alone — not from the publisher, the organization, or a timestamp — so a Performer can recompute it from the files on its own disk and the same set never has two names. An empty entry list is not signable, so "no baseline" can never collide with one. | `src/baseline.rs`, `src/operations/baseline.rs` |
| BR-014 | Authoring code and ordering it run are two powers held by two principals: a node holding a baseline publisher key cannot record a Performer peer and a node with any non-revoked Performer peer cannot create one, and rollback is a local act with no node-to-node message kind that could ask a Performer to change version. | `src/node_registry.rs`, `src/baseline_publisher.rs`, `docs/internal/baseline-delivery.md` |
